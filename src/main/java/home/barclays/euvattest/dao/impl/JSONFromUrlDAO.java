...
public class JSONFromUrlDAO implements DataAccessObject<VatRate> {
    ...
    public List<VatRate> getAll() {
        try {
            JsonNode rootNode = objectMapper.readTree(new URL((String) PropertiesUtils.getProperties().get("hostUrl")));
            return objectMapper.readValue(rootNode.get((String)PropertiesUtils.getProperties().get("parsableNodeName")).toString(),
